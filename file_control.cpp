...
class myFileSystem
{ // class thats holds all the functions for controlling the file system within
public:
    ...
    int create(char name[16], int size)
    {
        ...
        for (int i = 0; i < 128; i++)
        { // for loop that checks number of open blocks to see if space available
            ...
            if (open_space == size)
            { // dont need to check rest of blocks, already found enough space
                break;
            }
        }

        if (open_space < size)
        { // means less blocks open than new file requires
    ...
    void ls()
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            if (isOpen == 1)
            { // means there is file and space is taken
    ...
    int writeFile(char filename[16], int blockNum, char buf[1024])
    {
        ...
        for (int i = 0; i < 16; i++)
        { // check all the inodes for the file to be written to
            char file[16];
            char fileSize[4];
            char blockWrite[4];
            int fs;
    ...
    int readFile(char filename[16], int blockNum)
    {
        ...
        { // check all the inodes for the file to be written to
            ...
            { // means file was found
                ...
                std::cout << "Block accessed for reading. Contents of block: " << readInfo << std::endl
                          << std::endl;
                return 1;
            }
            else
...
int main()
{
    ...
    while (!infile.eof())
    {
        ...
        std::cout << "Next operation: " << mode << std::endl
                  << std::endl;
